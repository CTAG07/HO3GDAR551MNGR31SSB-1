...
class omen_kickoff():
    ...
    def run(self, agent):
        ...
        #checking wich kickoff we're using
        if agent.me.location == Vec3(-2048, -2560, agent.me.location.z) or Vec3(-2048, 2560, agent.me.location.z):
            self.front_diagonal_right()
        elif agent.me.location == Vec3(2048, -2560, agent.me.location.z) or Vec3(2048, 2560, agent.me.location.z):
            self.front_diagonal_left()
        elif agent.me.location == Vec3(-256.0, -3840, agent.me.location.z) or Vec3(-256.0, 3840, agent.me.location.z):
            self.back_diagonal_right()
        elif agent.me.location == Vec3(256.0, -3840, agent.me.location.z) or Vec3(256.0, 3840, agent.me.location.z):
            self.back_diagonal_left()
        elif agent.me.location == Vec3(0.0, -4608, agent.me.location.z) or Vec3(0.0, -4608, agent.me.location.z):
            self.straight_kickoff()


    def straight_kickoff(self, agent):
        if agent.me.location.x == 0 and agent.me.veocity == 900:
            self.front_flip()
        if (agent.me.location - agent.ball.location).magnitude() < 400:
            self.front_flip()

    def front_diagonal_right(self, agent):   
        if (agent.me.location - agent.ball.location).magnitude() < 500:
            self.diagonal_flip_right()

    def front_diagonal_left(self, agent):   
        if (agent.me.location - agent.ball.location).magnitude() < 500:
            self.diagonal_flip_left()

    def back_diagonal_left(self, agent):
        if self.team == 0:
            target_location = Vec3(0.0, -2816.0, 70.0)
            while agent.me.location.y < target_location.y:
                self.controls.steer = steer_toward_target(agent.me, target_location)
                continue
    ...
    def back_diagonal_right(self, agent):
        if self.team == 0:
            target_location = Vec3(0.0, -2816.0, 70.0)
            while agent.me.location.y < target_location.y:
                self.controls.steer = steer_toward_target(agent.me, target_location)
