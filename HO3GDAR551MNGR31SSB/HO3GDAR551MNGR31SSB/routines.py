...
class omen_kickoff():
    ...
    def run(self, agent):
        ...
        side = 2 * agent.team - 1   # -1 for blue, +1 for orange
        y = abs(agent.me.location.y)
        left = agent.me.location.x * side < 0
        if y <= 3500 and left:
            self.front_diagonal_left()
        elif y <= 3500 and not left:
            self.front_diagonal_right()
        elif y <= 4000 and left:
            self.back_diagonal_left()
        elif y <= 4000 and not left:
    ...
    def straight_kickoff(self, agent):
        if agent.me.location.x == 0 and agent.me.velocity == 900:
            self.front_flip()
